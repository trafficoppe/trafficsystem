...
window.onload = function() { 
    ...
    if(mainCategory === 'fire') {
        document.getElementById('menu-fire').classList.add('active');
        ...
        if (currentUnit === 'all' || currentUnit === '') {
            ...
            if (currentUnit === 'ถังดับเพลิง') {
                document.getElementById('menu-fire-extinguisher').classList.add('active');
                ...
                document.getElementById('menu-fire-hose').classList.add('active');
                document.getElementById('page-title').innerText = 'อุปกรณ์ดับเพลิง: สายส่งน้ำดับเพลิง';
                currentSubFilter = 'สายส่งน้ำดับเพลิง';
            } else if (currentUnit === 'หัวฉีด') {
                document.getElementById('menu-fire-nozzle').classList.add('active');
        ...
        document.getElementById('menu-mission').classList.add('active');
        ...
        if (currentUnit.includes('ตารางเวร')) { 
            document.getElementById('menu-mission-schedule').classList.add('active');
            document.getElementById('page-title').innerText = 'ตารางเวรปฏิบัติงาน';
        } else {
            document.getElementById('filter-group-mission').style.display = 'flex';

            if(currentUnit.includes('ตรวจพื้นที่')) {
                document.getElementById('menu-mission-inspect').classList.add('active');
                ...
                document.getElementById('menu-mission-help').classList.add('active');
                ...
                document.getElementById('menu-mission-traffic').classList.add('active');
        ...
        if(currentUnit.includes('ยุทธศาสตร์')) {
            document.getElementById('menu-staff-strat').classList.add('active');
            ...
            document.getElementById('menu-staff-op').classList.add('active');
        ...
        document.getElementById('filter-group-staff').style.display = 'flex';
    }
...
function applyFilters() {
    ...
    if (mainCategory === 'staff') {
        ...
        if (currentUnit.includes('ตรวจพื้นที่')) {
            filteredData = filteredData.filter(v => v.Data_Category === 'Mission' && v.ประเภท && v.ประเภท.includes('ตรวจพื้นที่'));
        } else if (currentUnit.includes('ช่วยเหลือ')) {
            filteredData = filteredData.filter(v => v.Data_Category === 'Mission' && v.ประเภท && v.ประเภท.includes('ช่วยเหลือ'));
        } else if (currentUnit.includes('ตารางเวร')) {
            filteredData = filteredData.filter(v => v.Data_Category === 'Schedule'); 
        } else if (currentUnit.includes('การจราจร')) {
            filteredData = filteredData.filter(v => v.Data_Category === 'Mission' && v.ประเภท && v.ประเภท.includes('การจราจร'));
        } else {
            filteredData = filteredData.filter(v => v.Data_Category === 'Mission');
        }
        // 🌟 พลิกข้อมูลหน้ารายงาน/ตรวจพื้นที่ ให้อันล่าสุดอยู่บนสุด
...
function displayGallery(dataToDisplay) {
    ...
    dataToDisplay.forEach((item) => {
        ...
        if (item.Data_Category === 'Staff') {
            ...
            let dateObj = new Date(item.สี);
            if(!isNaN(dateObj)) {
                subtitle = dateObj.toLocaleString('th-TH', { dateStyle: 'medium', timeStyle: 'short' });
            }
        }
...
function openModal(index) {
    ...
    let autoFillURL = `index.html?type=${encodeURIComponent(vehicle.ประเภท)}&detail=${encodeURIComponent(vehicle['ยี่ห้อ/รุ่น'])}&color=${encodeURIComponent(vehicle.สี)}&plate=${encodeURIComponent(vehicle.ทะเบียน)}&equip=${encodeURIComponent(vehicle.หมายเลขครุภัณฑ์)}`;
    ...
    if (vehicle.Data_Category === 'Staff') {
        ...
        let dateObj = new Date(vehicle.สี);
        let timeString = isNaN(dateObj) ? vehicle.สี : dateObj.toLocaleString('th-TH', { dateStyle: 'full', timeStyle: 'medium' });
...
window.openRefillHistoryModal = function(index) {
    ...
    // 🌟 พลิกข้อมูลให้แถวล่าสุด (ท้ายชีท) ขึ้นมาอยู่บนสุดเสมอ 🌟
