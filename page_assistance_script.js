...
const assistSheetUrl = "https://docs.google.com/spreadsheets/d/1lYRhXtLgec6ISM6Ugt-YKLrh47NRt7ihcVLcD8mI_Yg/gviz/tq?tqx=out:json&gid=1509680619";
...
function injectUI() {
    ...
    const headerTemplate = document.getElementById('assistFilterTemplate');
    ...
    if (headerSlot && headerTemplate) {
        ...
        if (searchInput) searchInput.oninput = displayAssistData;
    }
}
...
function fetchAssistData() {
    const loadingEl = document.getElementById('assetLoading');
    if(loadingEl) loadingEl.style.display = 'block';

    fetch(assistSheetUrl)
        .then(res => res.text())
        .then(text => {
            const match = text.match(/google\.visualization\.Query\.setResponse\(([\s\S\w]+)\);/);
            if (match && match[1]) {
                const data = JSON.parse(match[1]);
                allAssistData = data.table.rows.map((row, index) => {
                    ...
                    if(!category.includes('ช่วยเหลือ')) return null;
                    ...
                    return { 
                        date: dateStr,
                        displayDate: displayDate, // 🌟 ใช้ตัวนี้โชว์ในระบบ
                        fileName: fileName,
                        image: imageUrl,
                        category: category,
                        timestamp: timestamp,
                ...
                allAssistData.sort((a, b) => {
                    if (b.timestamp !== a.timestamp) return b.timestamp - a.timestamp;
                    return b.rowIndex - a.rowIndex;
                });

                // จำกัด 20 รายการล่าสุด
                allAssistData = allAssistData.slice(0, 20);
...
function displayAssistData() {
    const gallery = document.getElementById('assistGallery');
    if(!gallery) return;
    ...
    if (filtered.length === 0) {
        gallery.innerHTML = `<div class="loading-text" style="grid-column: 1/-1;">🔍 ไม่พบข้อมูลการให้ความช่วยเหลือ</div>`;
        return;
    }
    ...
    gallery.innerHTML = filtered.map(v => {
        ...
        <div class="vehicle-card" style="background: #ffffff; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1); overflow: hidden;">
            ...
            <div class="card-content" style="padding: 15px 20px; text-align: left;">
                <h3 class="card-title" style="color: #000000; font-size: 16px; margin: 0 0 10px 0; font-weight: 600; word-break: break-word;">${v.fileName || 'ภาพถ่ายการช่วยเหลือ'}</h3>
